...
%% Uncomment below to include a teaser figure.
   \teaser{
   \centering
   \includegraphics[width=16cm]{teaser.pdf}
...
The amino acids in red and blue illustrate an individual interacting pairs in the contact zone.

\begin{figure}[bt]
  \centering
  \includegraphics[width=0.8\columnwidth]{dockscore.png}
  \caption{Visual representation of the native and computed conformations of the protein complex with PDB ID 1SYX. Image taken from the work of Malhotra et al.~\cite{Malhotra2015}.}
  \label{fig:dockscore}
\end{figure}
...
\begin{figure}[tb]
  ...
  \caption{Superposition of several possible conformations between two proteins visualized using traditional cartoon rendering. The set of green protein instances corresponds to one of the interacting proteins, the colored components represent the second protein in different conformations.}
  \label{fig:problem}
\end{figure}
...
They aim to remove the problems of existing solutions and provide the domain experts with an intuitive and user friendly tool for an interactive exploration of protein-protein interactions.
These are coming from the computational tools.
...
Generally, molecular visualization is a very important and useful topic in visualization.
This was confirmed also by Kozlikova, Krone et al.~\cite{Kozlikova2015} in their survey of molecular visualization techniques.
...
However, the problem of the presented solution lies mainly in the missing information about the interacting amino acids and the unified coloring of the contact zones.
...
\begin{figure}[bt]
  ...
  \caption{(a) Surface visualization of two interacting proteins with PDB ID 4SGB where the contact zone is not visible. (b) and (c) Visualization of the intermolecular negative surface representing the volume between the surfaces of the contact zone. Image taken from Lee and Varshney~\cite{Varshney2003}.}
  \label{fig:varshney}
\end{figure}
...
Bruckner et al.~\cite{Bruckner2006} adopted this technique to volume data and demonstrated it on the scans of different parts of the human body.
...
Byska et al.~\cite{Byska2015} proposed visual abstractions of protein tunnels serving for ligand transportation and their evolution in molecular dynamics simulations.
...
Lex et al.~\cite{Lex2012} proposed a visual analysis tool serving for the exploration of large-scale heterogeneous genomics data for characterization of cancer subtypes.
They use different views onto the complex data and one of them is a method for the comparison of different datasets.
The abstracted representation shows the similarities in the datasets by connecting the corresponding blocks of data. 
The thicknesses of the connections denote the degree of similarities.
This representation is to some extent similar to our proposed InCo lens view but in our case we primarily focus on the exploration of the interacting amino acids in individual conformations.
...
Generally, there can be $n$ possible conformations for a given complex ($1 \leq i \leq n$) and the task is to select the conformation which is the most relevant one from a biochemical point of view.
...
Therefore, the users have to iteratively filter out those conformations which are not fulfilling given specific criteria.
...
This means that the experts are able to define a pair of amino acids from the interacting proteins that should interact as well.
This information can be set as an input parameter directly to the selected computational tool.
...
Our methods were designed in order to help the domain expert to answer the following basic questions.
\begin{itemize}
...
\item Q5: How are the contact zones arranged and how different are they?
\item Q6: What about different properties of the amino acids in the contact zone?
\item Q7: How close are the amino acids in the contact zone?
\item Q8: How exactly does the contact zone of a selected conformation look like? How is the correspondence between its amino acids?
\item Q9: What is the difference between the sets of amino acids in the contact zones of different conformations?
\end{itemize}
...
When using a computational tool for generating possible conformations, the resulting set $CONF_i(C(P_1,P_2))$ can be very large, where $i$ spans from dozens to hundreds, and requires some preliminary filtering.
This filtering stage is based on answering questions Q1 and Q2.
To answer them, we propose a matrix-based visualization inspired by commonly used heat maps (see Figure~\ref{fig:matrixlens}a).
...
The color of each cell in the matrix corresponds to the number of occurrences of the corresponding interacting amino acids in the set $S$ of all conformations, $S = \{CONF_i(C(P_1,P_2)); 1 \leq i \leq n\}$. 
...
\begin{figure}[bt]
  ...
  \caption{(a) CMV showing the aggregated information about the presence of mutually interacting amino acids in all conformations. It is interactively linked with (b) the InCo lens view presenting more detailed information about individual conformations and their interacting amino acids.}
...
\begin{figure}[hbt]
  ...
  \caption{CMV sorted according to the frequency of occurrence of the amino acids in all conformations. Green borders of some matrix cells represent the pairs which are present in some of the conformations (selected in the InCo lens view). Selected cells are marked with a cross. Table lens technique enables to enlarge a selected cell.}
  \label{fig:sort}
\end{figure}

One of the biggest advantages of the CMV is its independence on the size of the input set of possible conformations.
...
Then, each conformation of the input dataset increases the counters in the matrix cells.
Therefore, the only parameter influencing the size of the matrix is the size of the interacting proteins.
For large proteins the cells of the matrix can be too small.
In such situations, the users can employ the table lens technique introduced by Rao and Card~\cite{Rao1994} applied to both rows and columns of the matrix.

The CMV is interactively linked with another newly proposed visualization, i.e, the InCo lens view, which is positioned directly next to the conformation matrix (Figure~\ref{fig:matrixlens}b). 
...
The central part consists of rectangles representing individual conformations of a subset of $S$ where the rectangle in the middle corresponding to the conformation in focus gets the largest screen space.
...
These lists are the same as in the case of the CMV, i.e., they contain all amino acids present at least in one contact zone from $S$.
The amino acids are again colored according to the frequency of their occurrence within $S$.

By default each conformation in focus contains one polyline connecting two amino acids from the contact zone which are in the closest distance among all the possible pairs of amino acids.
...
To overcome these limitations, we adapt the technique of so called exploded views which enlarges the distance between the interacting proteins (see Figure~\ref{fig:exploded}). 
...
One of the $P_1$ and $P_2$ proteins has to be marked as the \textbf{reference protein} $P_{ref}$.
This can be done automatically (by setting the protein loaded first as the reference one) or the user can select which of the loaded proteins should be the reference one.
The set of reference proteins is then marked as $S(P_{ref})$.
Then the second protein from $C(P_1,P_2)$ is automatically marked as the \textbf{paired protein} $P_{pair}$ and the set of these proteins is marked as $S(P_{pair})$.
...
Now we have the necessary terminology to describe the main principle of the CEV.
First all the $S(P_{ref})$ reference proteins taken from the conformations selected by the CMV and the InCo view are aligned using a Combinatorial Extensions structural alignment algorithm~\cite{Shindyalov1998} (see the green proteins in Figure~\ref{fig:problem}).
...
Thus, a "reshuffling" of the $P_{pair}$ proteins has to be applied to avoid these collisions. 
We apply a heuristic but effective approach when all $P_{pair}$ proteins are put into a queue.
...
To avoid deadlocks, the whole reshuffling process is repeated until no collisions are detected any more.
...
This is important because if several conformations are exploded at once the CEV contains many $P_{pair}$ proteins arranged around the aligned $P_{ref}$ proteins and the pairing information is completely lost.
Therefore, we display the connection between corresponding proteins $P_{ref}$ and $P_{pair}$ as a tube connecting the centers of the parts $CZ(P_{ref})$ and $CZ(P_{pair})$ of the contact zone.
The radius of the tube at each point of the segment is modulated by its distance to the segment endpoints.
It is smaller in the middle of the tube to reduce visual clutter.
The opacity of the tube is modulated in screen space, depending on the distance to the segment centerline.

Figures~\ref{fig:case1} and~\ref{fig:case2} depict a set of five conformations before and after using the CEV.
The exploded view removes not only the problem of overlapping paired proteins but also enabled the user to see the contact zones which were in the original position of the interacting proteins hidden between them.
However, this solution does not solve the problem that the contact zones are still facing each other so the user has to adjust the camera to observe $CZ(P_{ref})$ or $CZ(P_{pair})$ from a perpendicular direction. 
Even such a manipulation does not enable to see $CZ(P_{ref})$ and $CZ(P_{pair})$ simultaneously.
This problem is solved by the Conformation Open-Book view presented in the following section.

\section{CoOB -- Conformation Open-Book View}
As stated above, the CEV suffers from the problem of an intuitive navigation through the space to observe the $CZ(P_{ref})$ and $CZ(P_{pair})$ parts of the contact zones in detail and at once.
The CoOB view is designed specifically to answer questions similar to Q8 which are dealing with the problem of a detailed exploration of a selected contact zone $CZ(P_1,P_2)$ of the complex $C(P_1,P_2)$.
...
The CoOB view is activated if the user selects one of the conformations from the exploded view. 
...
Within the animation the $P_{ref}$ and $P_{pair}$ proteins are rotated and translated so that they are positioned next to each other and the $CZ(P_{ref})$ and $CZ(P_{pair})$ parts of the contact zone are facing towards the observer (see Figure~\ref{fig:book}). 
...
The vector between $A_{CZ}$ and $B_{CZ}$ in fact corresponds to the centerline of the tube connecting $P_{ref}$ and $P_{pair}$ in the CEV.
Then we compute the normal vectors of both $CZ(P_{ref})$ and $CZ(P_{pair})$ as $N_A = B_{CZ} - A_{CZ}$ and $N_B = A_{CZ} - B_{CZ}$.
...
The color can correspond to the mutual distance between the amino acids, i.e., the closest amino acids have a different color than the most distant ones.
...
To inform the users about the types and identifiers of individual amino acids, the surfaces can be augmented with labels carrying this information.
...
Moreover, these methods can be combined so, for example, the proteins can be represented by cartoon method and the amino acids in the contact zones can be visualized using the sticks representation to see their spatial orientation.% (see Figure~\ref{fig:contact}).

The combination of the CEV and CoOB view is very useful for the exploration of individual conformations.
...
The order depends on the current task, i.e., if we want to compare the constitution of $CZ(P_{ref})$ or $CZ(P_{pair})$ in the given conformations.
The view contains all possible connections (wrt. to the distance) between amino acids from both contact zones.
To avoid intersection of lines representing the connections, some amino acids on the right side are repeated -- one occurrence for each amino acid in the reference protein within a distance of 5 \AA. 
This solution was adopted because of many intersections in the solution without the repetition of the amino acids.

\begin{figure}[bt]
  \centering
  \includegraphics[width=0.6\columnwidth]{list.png}
  \caption{Compact mode of the CoLi view (a) showing all pairs of amino acids from interacting proteins which are in interaction distance. The CoLi view (b) serves better for comparison of different conformations. It preserves the blank space for amino acids which are in the contact zone in some of the conformations but not in this particular one.}
  \label{fig:list}
\end{figure}
...
\begin{figure}[bt]
  ...
  \caption{Sorting of the CoLi view according to different properties of amino acids -- (a) hydrophobicity, (b) mutual distance, (c) frequency of occurrence.}
  \label{fig:sorting}
\end{figure}

The main principle of the algorithm building the CoLi view is the following.
For all conformations which should be visualized in the CoLi view and all amino acids in their contact zones we find the closest interacting pair of amino acids.
It is the same pair as the one represented by the default polyline in the InCo view.
Then the list of amino acids present in all $CZ(P_{ref})$ of the selected conformations is created.
Now, for each conformation, we take the closest amino acids detected in the first step, sort them according to a selected criterion, and plot them to the CoLi view.
To enable comparison the amino acids in the left part of the CoLi view are always sorted in the same way for all depicted conformations.
The user can select between two modes -- the compact list view and the compare list view.
The difference lies in the positioning of the amino acids in the left part of the CoLi view.
In the compare mode, each amino acid has a fixed position and if this amino acid is not present in the contact zone for some conformation, it is depicted as a white rectangle with the name of this missing amino acid as label (see Figure~\ref{fig:list}b).
Because the comparison and sorting is performed only in the left part of the CoLi view, these white rectangles occur only there.
The compact mode ignores the missing amino acids and arranges the amino acids in the most compact form to save space (Figure~\ref{fig:list}a, right part).
...
This means that by the selection of cells in the CMV the user automatically filters out those conformations which do not fulfill the filter criteria and these conformations are further omitted from the other views as well.
Of course, the user can at anytime return to the beginning of the process. 
By the iterative refinement of the filtering using the matrix and lens views the set of conformations can be further reduced.
...
Such conformations can then be included back into the exploration process.
...
The crystal structure of the complex represents the dimer of kite proteins which are critical for function of the SMC5/6 (structure maintenance of chromosome) complex~\cite{Zabrady2016}. 
...
All computed conformations were loaded into our visualization system incorporating all proposed visualizations.
...
Even as the user specifies the input pair of amino acids, only a small number of the resulting conformations really contain this interaction.
...
In our case study the InCo view helped the biochemist to confirm the initial selection made through the CMV.
In all five conformations it confirmed that the most frequent amino acids are in the interaction distance.
Thus there is a high probability that these will also occur in the contact zone of the resulting biochemically relevant conformation.

In the next step, the five conformations were explored in the 3D space.
...
However, the most interesting parts, i.e., the contact zones, are completely hidden.

\begin{figure}[bt]
  \centering
  \includegraphics[width=0.7\columnwidth]{case1.png}
  \caption{Five conformations represented by surfaces with highlighted contact zones. It is obvious that such a representation is insufficient for the observation and comparison of the contact zones.}
  \label{fig:case1}
\end{figure}
...
Moreover, if the user is directing the camera towards the $P_{ref}$, the differences between the positions of the contact zones in the $P_{ref}$'s can be observed as well.
The connection tubes were appreciated by the domain expert
If rotating the scene the user can easily lose the information about the direction of the shift between the proteins.
The CEV representation gave the domain expert very interesting information about the mutual positioning of the individual conformations with respect to the positions of the contact zones.

\begin{figure}[bt]
  \centering
  \includegraphics[width=0.95\columnwidth]{case2.png}
  \caption{Five conformations represented by a conformation exploded view (CEV).}
  \label{fig:case2}
\end{figure}

However, the investigation has to go more deeply to the level where individual contact zones can be explored in detail.
In this case each conformation can be explored individually using the CoOB view.
...
The enhancements of the CoOB view, i.e., the possibility to see the connection lines between the interacting amino acids, the labeling of the surface of the contact zones with the names of these amino acids, and the coloring according to different criteria, were highly appreciated by the domain expert.
This helped to clearly understand the constitution of the contact zone and all its properties.
After the exploration using the CoOB view the expert returned to the unopened view and used the possibility to visualize the polypeptidic chains of the interacting proteins in a more abstracted way employing the cartoon representation. 
This allowed him to add the detailed representation of interesting pairs of amino acids selected using the CoOB view.
...
From this view it is clearly visible that the contact zone of the conformation $CONF_5(C(P_{ref},P_{pair}))$ contains much less amino acids in $P_{ref}$ than the other conformations. 
...
At the end of this process the domain expert confirmed that he obtained enough relevant information to decide that the conformation $CONF_5(C(P_{ref},P_{pair}))$ is not the relevant one and the remaining, very similar, conformations are worthwhile to test in the laboratory.
...
He affirmed that using our solution their exploration process can lead to a satisfying conclusion about the biochemical relevance of individual conformations much faster.
The system enables to iteratively filter out conformations not satisfying given conditions in individual stages of the workflow.

In the future we will focus on the extension of our proposed techniques to more complicated cases where the user has no a priori knowledge about the protein complex.
